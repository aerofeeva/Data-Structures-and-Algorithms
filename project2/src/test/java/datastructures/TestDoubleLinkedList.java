...
public class TestDoubleLinkedList extends BaseTest {
    ...
    // Start of new code
    @Test(timeout=SECOND)
    public void testDeleteFrontAndEnd() {
        IList<String> list = this.makeBasicList();
        
        String first = list.delete(0);
        assertEquals("a", first);
        assertListMatches(new String[] {"b", "c"}, list);
        
        String last = list.delete(1);
        assertEquals("c", last);
    ...
    public void testDeleteMiddle() {
        ...
        String middle = list.delete(1);
        assertEquals("b", middle);
        assertListMatches(new String[] {"a", "c"}, list);
    }
    
    @Test(timeout=SECOND)
    public void testDeleteIndexOutofBounds() {
        IList<String> list = this.makeBasicList();
    ...
    public void testDeleteFromFrontIsEfficient() {
        ...
        int first = list.delete(0);
        assertEquals(0, first);
        assertEquals(cap - 1, list.size());
    }
    
    @Test(timeout=15 * SECOND)
    public void testDeleteFromMiddleIsEfficient() {
        IList<Integer> list = new DoubleLinkedList<>();
        int cap = 5000000;
        for (int i = 0; i < cap; i++) {
            list.add(i);
        }
        int middle = list.delete(list.size()/2);
        assertEquals((list.size() + 1) / 2, middle);
        assertEquals(cap - 1, list.size());
    }
    ...
    public void testDeleteFromEndIsEfficient() {
        ...
        int last = list.delete(list.size() - 1);
        assertEquals(list.size(), last);
        assertEquals(cap - 1, list.size());
    }
    ...
    @Test(timeout=SECOND)
    public void testAddAndDeleteFromFront() {
        IList<Integer> list = new DoubleLinkedList<>();
        int cap = 1000;
    ...
    @Test(timeout=5 * SECOND)
    public void testInsertAndDeleteFromMiddleIsEfficient() {
        IList<Integer> list = new DoubleLinkedList<>();
        for (int i = 0; i < 10000; i++) {
            list.add(i);
        }
        ...
        for (int i = 0; i < 10000; i++) {
            list.insert(list.size()/2, -1);
            list.delete((list.size() - 1)/2);
        }
